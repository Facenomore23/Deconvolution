...
def parse_data(x_path, y_path):
    ...
    # Load the data
    print(x_path)
    x = pd.read_table(x_path, sep="\t", index_col=0)
    y = pd.read_table(y_path, sep="\t",index_col=0)
...
print(glob.glob(data_dir + pattern))
...
for i, train_file in enumerate(datasets):
    ...
    ratios = pd.DataFrame(y, columns=labels)
