...
class Scaden(object):
    ...
    def compute_accuracy(self, logits, targets, pct_cut=0.05):
        ...
        :return:
        """
        equality = tf.math.less_equal(tf.math.abs(tf.math.subtract(logits, targets)), pct_cut)
    ...
    def visualization(self, logits, targets, classes):
        ...
        tf.print("tensor logits :", logits, output_stream=sys.stdout)
        rmse = tf.metrics.RootMeanSquaredError()
        rmse.update_state(logits, targets)
        ...
        for i in range(logits.shape[1]):
            eval_metrics["mre_" + str(classes[i])] = tf.metrics.MeanRelativeError(normalizer=targets[:, i])
            eval_metrics["mre_" + str(classes[i])].update_state(
                targets[:, i],
                logits[:, i])
            ...
            #    logits[:, i],
            #    targets[:, i])[0]
            eval_metrics["mae_" + str(classes[i])] = tf.metrics.MeanAbsoluteError()
            eval_metrics["mae_" + str(classes[i])].update_state(
                targets[:, i],
                logits[:, i])
        ...
        eval_metrics["mre_total"] = tf.metrics.MeanRelativeError(targets)
        eval_metrics["mre_total"].update_state(targets, logits)
                                                                   
                                                            

        eval_metrics["mae_total"] = tf.metrics.MeanRelativeError(targets)
        eval_metrics["mae_total"].update_state(targets, logits)
    ...
    def load_prediction_file(self, input_path, sig_genes, labels, scaling=None):
        ...
        self.data = tf.data.Dataset.from_generator((self.x_data_ph, self.y_data_ph))
    ...
    def build_model(self, input_path, train_datasets, mode="train"):
        ...
        # Make iterator
        iter = tf.compat.v1.data.Iterator.from_structure(self.data.output_types, self.data.output_shapes)
